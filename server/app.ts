...
app.get("/", async (req, res) => {
  try {
    const fluxQuery = 'from(bucket:"' + bucket + '") |> range(start: 0)';
...
app.get("/engine", async (req, res) => {
  try {
    const fluxQuery =
      'from(bucket:"' +
      bucket +
      '") |> range(start: 0)  |> filter(fn: (r) => r._measurement == "engine-1" or r._measurement == "engine-2" or r._measurement == "engine-3") |> window(every: 1s) |> mean() ';
    const query = res.json(await queryApi.collectRows(fluxQuery));
    res.send(query);
  } catch (error) {
    console.log(error.stack);
